 ...
 * @author zeyad
 */
public class TesterPerformance extends javax.swing.JFrame {

    private final Statement statement;
    private final int id;
    public TesterPerformance(int id) {
        this.id=id;
    ...
    private void Show() {
        String sql = "SELECT * FROM TesterPerformance";
        try {
            ResultSet rs = this.statement.executeQuery(sql);
            DefaultTableModel df = (DefaultTableModel) this.table_.getModel();
            df.setRowCount(0);
            while (rs.next()) {
                df.addRow(new Object[]{rs.getString("ID"), rs.getString("name"), rs.getString("no_bugs_discovered"), rs.getString("meets_the_target")});
            }
    ...
    private void initComponents() {
        ...
        table_.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Name", "Number of Bugs Discovered", "Meets The Target"
            }
        ) {
                ...
                switch (rs.getString(4)) {
                    ...
                    case "pm" -> {
                        ...
                        java.awt.EventQueue.invokeLater(new Runnable() {
                            @Override
                            public void run() {
                                new ProjectManager(name, id).setVisible(true);
                            }
                        });
                        this.dispose();
                    }
                    ...
                    case "developer" -> {
                        ...
                        java.awt.EventQueue.invokeLater(new Runnable() {
                            @Override
                            public void run() {
                                new Developer(name, id).setVisible(true);
                            }
                        });
                        this.dispose();
                    }
                    ...
                    case "tester" -> {
                        ...
                        java.awt.EventQueue.invokeLater(new Runnable() {
                            @Override
                            public void run() {
                                new Tester(name, id).setVisible(true);
                            }
                        });
                        this.dispose();
                    }
